...
import { User, Mail, Phone, Calendar, MapPin, Edit3, Save, X, Stethoscope, Award, Clock, Briefcase, GraduationCap } from 'lucide-react'
...
const DoctorProfile = () => {
    ...
    if (loading) {
        return <Layout><div className="flex items-center justify-center min-h-screen"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600"></div></div></Layout>
    }
                ...
                <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
                    <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col md:flex-row items-center gap-8 mb-8">
                        ...
                        <div className="text-center md:text-left">
                            ...
                            <p className="text-lg text-blue-600 font-semibold mt-1">{profileData?.specialization || 'Chuyên khoa'}</p>
                        </div>
                        <div className="md:ml-auto flex gap-2 mt-4 md:mt-0">
                            {!isEditing ? (
                                <Button onClick={() => setIsEditing(true)} className="bg-blue-600 hover:bg-blue-700 text-white"><Edit3 className="h-4 w-4 mr-2" />Chỉnh sửa</Button>
                            ) : (
                                <>
                    ...
                    <Card className="shadow-xl border-0 bg-white">
                        <CardHeader>
                            <nav className="-mb-px flex space-x-8" aria-label="Tabs">
                                <button onClick={() => setActiveTab('personal')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'personal' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>Thông tin cá nhân</button>
                                <button onClick={() => setActiveTab('professional')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'professional' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>Thông tin chuyên môn</button>
                            </nav>
                        </CardHeader>
                        <CardContent className="p-6">
                            ...
                            {activeTab === 'professional' && (
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <ProfileField icon={<Stethoscope />} label="Chuyên môn" value={formData.specialization} isEditing={isEditing} name="specialization" onChange={handleInputChange} />
                                    ...
                                    <ProfileField icon={<Clock />} label="Giờ làm việc" value={formData.working_hours} isEditing={isEditing} name="working_hours" onChange={handleInputChange} />
                                    <div className="md:col-span-2">
                                    ...
                                    <div className="md:col-span-2">
                                        <ProfileField icon={<Award />} label="Bằng cấp" value={formData.degrees} isEditing={isEditing} name="degrees" onChange={handleInputChange} children={isEditing && <textarea name="degrees" value={formData.degrees} onChange={handleInputChange} rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-lg" />} />
                                    </div>
                                </motion.div>
                            )}
                        </CardContent>
