...
type PatientRepository interface {
	...
	UpdatePatient(ctx context.Context, id string, m *patient.Patient) error
...
func (r *patientRepo) GetPatientById(ctx context.Context, patientId string) (*patient.Patient, error) {
	p := &patient.Patient{}
...
func (r *patientRepo) UpdatePatient(ctx context.Context, patientId string, m *patient.Patient) error {
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
		logrus.Errorf("Failed to create transaction: %v", err)
		return err
...
func (r *patientRepo) DeletePatientById(ctx context.Context, patientId string) error {
...
func (r *patientRepo) migrate() error {
	...
	logrus.Info("Successfully migrated medical_history table")
	...
	logrus.Info("Successfully migrated general_examination table")
